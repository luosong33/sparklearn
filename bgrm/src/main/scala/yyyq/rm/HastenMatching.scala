...
object HastenMatching {

  def main(args: Array[String]): Unit = {
    var masterUrl = "local[10]" //  local[10]  yarn
    if (args.length > 0) {
      masterUrl = args(0)
    }
    // Create a StreamingContext with the given master URL
    val conf = new SparkConf().setMaster(masterUrl).setAppName("HastenMatching_node4_0305") // HastenMatching_date_node4_Transition
    ...
    val kafkaParams = Map[String, String](
      ...
      "group.id" -> "HastenMatching_node4_0305",
...
//        val lastString = DateUtil.nowString("yyyy-MM-dd HH:mm:ss")  //  最新日期
...
//            HttpClientUtil.sendHttpPostJson("http://riskdata-pool/dataDirect/operator", str, HttpClientUtil.JSON)
//            HttpClientUtil.httpPostJson(jSONObject, "http://riskdata-pool/dataDirect/operator")
