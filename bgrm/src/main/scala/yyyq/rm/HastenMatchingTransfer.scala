...
object HastenMatchingTransfer {
  @volatile private var broadcastValue: Broadcast[Connection] = null

  def main(args: Array[String]): Unit = {
    var masterUrl = "yarn" //  local[10]  yarn
    if (args.length > 0) {
      masterUrl = args(0)
    }
    // Create a StreamingContext with the given master URL
    val conf = new SparkConf().setMaster(masterUrl).setAppName("HastenMatching_date_node4_transfer")
    conf.set("spark.streaming.kafka.maxRatePerPartition", "10")
    conf.set("spark.streaming.backpressure.enabled", "true")
    val ssc = new StreamingContext(conf, Seconds(10))

    if (broadcastValue != null){
      broadcastValue = ssc.sparkContext.broadcast(inserOrUpdate())  //  广播变量  每个excoter一个
    }
    ...
    val kafkaParams = Map[String, String](
      ...
      "group.id" -> "HastenMatching_date_node2_history",
    ...
    val events = kafkaStream.map(line => {
      var clientNo = ""
      var creditNo = ""
      var phoneNum_10 = 0                       //  3千10天匹配催收库个数
      var callNum_10 = 0                        //  3千10天匹配催收库次数
      var phoneNum_20 = 0                       //  3千20天匹配催收库个数
      var callNum_20 = 0                        //  3千20天匹配催收库次数
      var phoneNum_30 = 0                       //  3千30天匹配催收库个数
      var callNum_30 = 0                        //  3千30天匹配催收库次数
      var phoneNum_90 = 0                       //  3千90天匹配催收库个数
      var callNum_90 = 0                        //  3千90天匹配催收库次数
      var phoneNum_180 = 0                      //  3千180天匹配催收库个数
      var callNum_180 = 0                       //  3千180天匹配催收库次数
      var origDate = ""                         //  原始时间
      var nowDate = ""                          //  入库时间
      var proportion30 = 0.00                   //  30天风险库通话占总通话人数比
      var top10Num = 0                          //  运营商通话记录top10撞中催收号码个数
      var phoneNum_90_fortyThousand = 0         //  4万90天匹配催收库个数
      var phoneNum_180_fortyThousand = 0        //  4万180天匹配催收库个数
      try {
        ...
        val lastString = streamingMap.last._1 //  最新日期
      ...
      rdd.foreachPartition(partitionOfRecords => {
        ...
        partitionOfRecords.foreach(pair => {
          try {
            ...
            var client_no = pair._1.split("\\|")(0)
            val conn = broadcastValue.value
              // Configure to be Read Only
              val statement = conn.createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY)
              val sql = "INSERT into tablename ( " +
                "phoneNum_10, callNum_10, phoneNum_20, callNum_20, phoneNum_30, callNum_30, phoneNum_90, callNum_90 " +
                "phoneNum_180, callNum_180, origDate, nowDate, proportion30, top10Num, phoneNum_180_fortyThousand, phoneNum_90_fortyThousand) " +
                " values ('" +strarr2(0)+ "','"+strarr2(1)+"','"+strarr2(2)+"','"+strarr2(3)+"','"+strarr2(4)+"','"+strarr2(5)+"','"+strarr2(6)+"','"+strarr2(7)+"'," +
                "'"+strarr2(8)+"','"+strarr2(9)+"','"+strarr2(10)+"','"+strarr2(11)+"','"+strarr2(12)+"','"+strarr2(13)+"','"+strarr2(14)+"','"+strarr2(15)+"') " +
                "WHERE client_no = '"+client_no+"' on duplicate key " +
                "UPDATE tablename SET " +
                "phoneNum_10 = '"+strarr2(0)+"', callNum_10 = '"+strarr2(1)+"' " +
                "phoneNum_20 = '"+strarr2(2)+"', callNum_20 = '"+strarr2(3)+"' " +
                "phoneNum_30 = '"+strarr2(4)+"', callNum_30 = '"+strarr2(5)+"' " +
                "phoneNum_90 = '"+strarr2(6)+"', callNum_90 = '"+strarr2(7)+"' " +
                "phoneNum_180 = '"+strarr2(8)+"', callNum_180 = '"+strarr2(9)+"' " +
                "origDate = '"+strarr2(10)+"', nowDate = '"+strarr2(11)+"' " +
                "proportion30 = '"+strarr2(12)+"', top10Num = '"+strarr2(13)+"' " +
                "phoneNum_180_fortyThousand = '"+strarr2(14)+"', phoneNum_90_fortyThousand = '"+strarr2(15)+"' " +
                "WHERE client_no = '"+client_no+"' "
              statement.executeUpdate(sql)
              /*val rs = statement.executeQuery("SELECT 手机号 FROM pb_tel_risk_lib  WHERE client_no = " +client_no+ ";")
              if(rs.next) {
                ...
                val sql = "UPDATE tablename SET " +
                  ...
                  "phoneNum_180_fortyThousand = '"+strarr2(14)+"', phoneNum_90_fortyThousand = '"+strarr2(15)+"' " +
                  "WHERE client_no = '"+client_no+"' ";
                val pps = conn.prepareStatement(sql)
                pps.executeUpdate()
              }else{
                val sql = "INSERT into tablename SET " +
                  "phoneNum_10 = '"+strarr2(0)+"', callNum_10 = '"+strarr2(1)+"' " +
                  "phoneNum_20 = '"+strarr2(2)+"', callNum_20 = '"+strarr2(3)+"' " +
                  "phoneNum_30 = '"+strarr2(4)+"', callNum_30 = '"+strarr2(5)+"' " +
                  "phoneNum_90 = '"+strarr2(6)+"', callNum_90 = '"+strarr2(7)+"' " +
