...
import yyyq.util.{DateUtil, HBaseUtils}
...
object HastenMatchingYarn {

  def main(args: Array[String]): Unit = {
    var masterUrl = "yarn" //  local[*]  yarn
    val conf = new SparkConf().setMaster(masterUrl).setAppName("HastenMatching_yarn") // HastenMatching_date_upgrade HastenMatching_date
    conf.set("spark.streaming.kafka.maxRatePerPartition", "4")
    conf.set("spark.streaming.backpressure.enabled", "true")
    val ssc = new StreamingContext(conf, Seconds(5))

    // Kafka configurations
    val topics = Set("MOXIE_CARRIER_ACCESS_SUMMIT_HISTORY")
