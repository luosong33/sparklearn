...
object HastenMatchingLocalNew {

  def main(args: Array[String]): Unit = {
    var masterUrl = "local[10]" //  local[20]  yarn
    // Create a StreamingContext with the given master URL
    val conf = new SparkConf().setMaster(masterUrl).setAppName("HastenMatching_node4_local_0322") // HastenMatching_date_upgrade HastenMatching_date
    conf.set("spark.streaming.kafka.maxRatePerPartition", "5")
    conf.set("spark.streaming.backpressure.enabled", "true")
    val ssc = new StreamingContext(conf, Seconds(5))

    // Kafka configurations
    val topics = Set("MOXIE_CARRIER_ACCESS_SUMMIT") //  _HISTORY
    ...
    val kafkaParams = Map[String, String](
      ...
      "group.id" -> "HastenMatching_node4_local_0322",
...
/*
        ...
        //  90天计算
        var streaming90List = ArrayBuffer[String]() // 90天借款人通话记录
