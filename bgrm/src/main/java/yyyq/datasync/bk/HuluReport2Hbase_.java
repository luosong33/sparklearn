...
public class HuluReport2Hbase_ {
...
//            file = new File(fileName);
            ...
            try {
                ...
                while ((tempString = reader.readLine()) != null) {
                    try {
                        ...
                        insertBachHbase("hulu_report", voucherNo, jarr, mobile); // bops_client_call_record_gourd bops_client_call_record_gourd_test

                        JSONObject person = (JSONObject) jobj.get("person");
                        String sql = "";
                        insert(sql, person, voucherNo); // bops_client_call_record_gourd bops_client_call_record_gourd_test
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
     ...
     * 入库
     */
    public static void insert(String SQL, JSONObject jsob, String voucherNo) {
        ...
        try {
            assemblePerson(jsob, voucherNo, stmt);
            stmt.executeUpdate();
            con.commit();
            con.close();
    ...
    //  组装插入语句
    private static void assemblePerson(JSONObject job, String voucherNo, PreparedStatement stmt) throws SQLException {
        stmt.setString(1, voucherNo);
        stmt.setString(2, String.valueOf(job.get("age")));
        stmt.setString(3, String.valueOf(job.get("city")));
        stmt.setString(4, String.valueOf(job.get("gender")));
        stmt.setString(5, String.valueOf(job.get("id_card_num")));
        stmt.setString(6, String.valueOf(job.get("province")));
        stmt.setString(7, String.valueOf(job.get("real_name")));
        stmt.setString(8, String.valueOf(job.get("region")));
        stmt.setString(9, String.valueOf(job.get("sign")));
        stmt.setString(10, String.valueOf(job.get("state")));
        stmt.setString(11, String.valueOf(job.get("status")));
        stmt.setString(12, DateUtil.nowString("yyyy-MM-dd HH:mm:ss,sss"));
    }

    //  接收数组入库
    public static void insertBachHbase(String tablename, String voucherNo, JSONArray jarr, String mobile_local) {
        PreparedStatement stmt = null;
        Connection con = GetConnection.getPhoenixConn();
        try {
            con.setAutoCommit(false);
    ...
    public static void main(String[] args) throws SQLException {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                System.out.println("=================Call=files=" + ++files+"==filepath=="+filePath);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
