...
public class MoxieFund2Hbase {
    ...
    private static void handleInsert(String filePath, String voucher_no, String client_no, String cert_no, String tempString) {
        try {
            ...
            Set<String> jobjSet = jobj.keySet();
            for (String jobjkey : jobjSet) {
                String s = jobj.getString(jobjkey);
                s = s.substring(0, 1);
                if ("{".equals(s)) {
                    JSONObject job = (JSONObject) jobj.get(jobjkey);
                    insertJSONObject2Hbase("mojie_report_fund_" + jobjkey, job, cert_no, voucher_no, client_no);
                }
    ...
    public static void main(String[] args) throws SQLException {
        ...
        String sql = "SELECT VOUCHER_NO,FILE_PATH FROM cdsp_hulu_access n WHERE QUERY_TYPE = 'MOXIE_FUND_REPORT' " +
        ...
        System.out.println(DateUtil.nowString() + " MoxieFund2Hbase导入耗时为： " + (endtime - starttime));
    }
...
//                filePath = "D:\\workspace\\数据样例\\moxie_fund_1229.json";  //  功能自测
                readFileByLines(filePath, voucher_no, client_no, cert_no);
                System.out.println(DateUtil.nowString() + " ==MoxieFund2Hbase2Hbase=files1====" + ++files + "===filepath===" + filePath);
            }
        }
    }
...
//                filePath = "D:\\workspace\\数据样例\\moxie_fund_1229.json";  //  功能自测
                readFileByLines(filePath, voucher_no, client_no, cert_no);
                System.out.println(DateUtil.nowString() + " ==MoxieFund2Hbase=files1====" + ++files + "===filepath===" + filePath);
            }
        }
    }
